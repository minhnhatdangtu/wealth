...
// ─── Per-type card style (Gradient + Accent) ──────────────────────────────────
function getCardStyle(type: string): string {
  const t = type.toLowerCase();
  if (t.includes('chứng khoán')) {
    return 'bg-gradient-to-br from-amber-600 to-amber-900';
  }
  ...
  if (t.includes('bất động sản')) {
    return 'bg-gradient-to-br from-[#182a5c] to-[#0a1530]';
  }
  if (t.includes('vàng')) {
    return 'bg-gradient-to-br from-yellow-600 to-orange-800';
  }
  return 'bg-gradient-to-br from-gray-700 to-gray-900';
}

// ─── Type-specific extra metrics (active only) ────────────────────────────────
...
function TransactionHistory({ assets }: { assets: any[] }) {
  if (assets.length === 0) {
    return <div className="py-8 text-center text-gray-400 text-sm italic">Chưa có dữ liệu.</div>;
  }
  return (
    <div>
...
export default function AssetDetailPage() {
  ...
  // Only active assets for performance metrics
